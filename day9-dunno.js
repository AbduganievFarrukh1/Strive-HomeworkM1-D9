(function injected(eventName, injectedIntoContentWindow) {
    ...
    for (let element of [
      ...
      if (!contentWindowDesc) continue;
  
      let getContentDocument = Function.prototype.call.bind(
        contentDocumentDesc.get
      );
      let getContentWindow = Function.prototype.call.bind(contentWindowDesc.get);
    ...
    if (typeof RealRTCPeerConnection != "undefined") {
      ...
      let normalizeUrl = (url) => {
        if (typeof url != "undefined") return RealString(url);
      };
      ...
      let protectConfiguration = (configuration) => {
        ...
        let iceServers = safeCopyArray(configuration.iceServers, (iceServer) => {
      ...
      let checkConfiguration = (peerconnection, configuration) => {
        if (configuration && configuration.iceServers) {
          for (let i = 0; i < configuration.iceServers.length; i++) {
            ...
            if (iceServer) {
              if (iceServer.url) checkUrl(peerconnection, iceServer.url);
      ...
      let WrappedRTCPeerConnection = function (...args) {
        ...
        let constraints;
        if (args.length > 1) constraints = args[1];
  
        let peerconnection = new RealRTCPeerConnection(
          configuration,
          constraints
        );
